...
Varyings LitPassVertex(Attributes input)
{
	...
	WindSettings wind = PopulateWindSettings(
		_WindAmbientStrength, _WindSpeed, _WindDirection, _WindSwinging,
		input.color[_VertexColorWindChannel], _WindObjectRand, _WindVertexRand,
		_WindRandStrength, _WindGustStrength, _WindGustFreq, _WindGustSpeed
	);

	BendSettings bending = PopulateBendSettings(
		_BendMode, input.color[_VertexColorBendingChannel],
		_BendPushStrength, _BendFlattenStrength, _PerspectiveCorrection
	);
	...
	vertexInputs.normalOS = lerp(
		vertexInputs.normalOS,
		normalize(vertexInputs.positionOS.xyz),
		_NormalSpherify * lerp(1, input.color[_VertexColorShadingChannel], _NormalSpherifyMask)
	);
	...
	// Vertex color
	output.color.rgb = ApplyVertexColor(
		input.positionOS.xyz,
		vertexData.positionWS.xyz,
		_BaseColor.rgb,
		input.color[_VertexColorShadingChannel],
		_OcclusionStrength,
		_VertexDarkening,
		posOffset
	);

	// Pass vertex color alpha-channel to fragment stage
	...
	half4 tangentWS = half4(vertexData.tangentWS.xyz, sign);
#endif
	...
	// Unity 6000-safe replacement for old OUTPUT_SH / OUTPUT_SH4 macro usage
	// This avoids macro signature mismatches across URP versions.
...
void ModifySurfaceData(Varyings input, out SurfaceData surfaceData)
{
	...
	// If MSAA is enabled an issue occurs where vertex interpolated values are abnormal
	...
	if (_ColorMapParams.x == 1)
	{
		...
		albedoAlpha.rgb = lerp(
			ApplyColorMap(input.positionWS.xyz, albedoAlpha.rgb, _ColorMapStrength),
			albedoAlpha.rgb,
			colorMapMask
		);
	}
	...
	if (_BendTint.a > 0 && (_BendPushStrength > 0 || _BendFlattenStrength > 0))
	{
		...
		albedoAlpha.rgb = lerp(
			albedoAlpha.rgb,
			albedoAlpha.rgb * _BendTint.rgb,
			saturate(bendingMask * heightGradient * _BendTint.a)
		);
	}
...
void PopulateLightingInputData(Varyings input, half3 normalTS, out InputData inputData)
{
	...
	// Using GetWorldSpaceViewDir returns a constant vector for ortho cameras
...
#if defined(DEBUG_DISPLAY) && UNITY_VERSION >= 202120
...
#endif
{
	...
	surfaceData.alpha = ComposeAlpha(
		surfaceData.alpha,
		_Cutoff,
		input.positionCS.xyz,
